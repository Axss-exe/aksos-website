...
function useCanvasField(active: Set<string>, activeEdges: readonly [string, string][], options?: { drift?: boolean; labels?: boolean; onPick?: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    const schedule = () => { if (stopped || !visibleRef.current) return; cancelAnimationFrame(raf); raf = requestAnimationFrame(draw) }; const visibilityObserver = new IntersectionObserver(([entry]) => { visibleRef.current = entry.isIntersecting; if (entry.isIntersecting) schedule(); else cancelAnimationFrame(raf) }); const resizeObserver = new ResizeObserver(() => schedule()); visibilityObserver.observe(parent); resizeObserver.observe(parent); schedule()
    return () => { stopped = true; cancelAnimationFrame(raf); visibilityObserver.disconnect(); resizeObserver.disconnect() }
